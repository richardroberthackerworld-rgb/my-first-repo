...
final class BandB
{
    ...
    {
        $out = []; $seen = [];
        /* MARKDOWN EMPHASIS IS NOT CHEMISTRY. buildSystemPrompt tells the model
           to bold its answer, so a balanced equation arrives as
           "**CH4 + 2O2 -> CO2 + 2H2O**" — and the asterisks broke the left side
           apart, leaving the fragment "2O2 -> CO2", which of course does not
           balance. A correct equation was reported as wrong, and a simple one
           like "**2H2 + O2 -> 2H2O**" produced no check at all.

           Because the prompt MANDATES the bold, this hit essentially every
           chemistry answer the site produced. */
        $md = preg_replace('/\*\*|__|\*/u', '', $md);
